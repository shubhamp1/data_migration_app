class ProvinceCountryValidator < ActiveModel::Validator
  ...
  def validate(record)
    record.errors.add(:country, 'must be either the United States or Canada') if record.country && Location::LOCATION_HASH.keys.exclude?(record.country.downcase)
    if record.province.present? && market_type_helper?(record)
      record.errors.add(:province, 'must be within United States or Canada') if Location::STATES_AND_PROVINCES_DOWNCASE.exclude?(record.province.downcase)
